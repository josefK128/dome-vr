...
const fsh = `
     ...
     uniform float uTime;        // ellapsed time for animation - usually ms.
     ...
     //sphere params
     float radius =  1.0;   //no effect ?!


     // distance - used by march
     //return length(max(abs(p_v)-b, 0.0));  //single-cube
     float _distance(vec3 p){
       //float d = length(p-center);  //center of rm-object, i.e. uPosition
       //float d = length(p) - radius;            //single sphere 

       float d = length(p-uPositions[0]);
       return d - radius;
     }
     ...
     float sphere_sdf(vec3 p, vec3 v, vec3 b){
       ...
       //q affects location rm-object, and 'fraying' of rm-obj (k>1)      
       //very small values of k (exp k=.1) slow the pulsating to near still.
       //swap- not orig
       vec3 q = fract(p - v) * (2.0 + .8*sin(uTime)) -1.0;   //k*sin k=.1 
       //vec3 q = fract(max(abs(p_v)-b, 0.0)) * (2.0 + 0.1*sin(uTime)) -1.0;
       
       
       //b affects shape of rm-bject      
     ...
     float march(vec3 eye, vec3 fwd){
         ...
         for (int i=0; i<32; i++) {       // 32 iterations
             ...
             vec3 b = vec3(.2,.1, .2);  //delta=> elongation of rm-object
             vec3 v = vec3(0.,0.,0.0);  //delta=>no effect?!
             float d = max(sphere_sdf(p,v,b),0.0);  
             t += d*0.5;
         }
         return t;
     }
...
//        uniforms:_uniforms};
export { fsh, uniforms };
//# sourceMappingURL=fsh_inf_rmsphere.glsl.js.map