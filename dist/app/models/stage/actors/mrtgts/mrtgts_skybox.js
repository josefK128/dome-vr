...
export const Skybox = class {
    static create(options = {}) {
        ...
        return new Promise((resolve, reject) => {
            const cube_g = new THREE.BoxGeometry(size, size, size, 1, 1, 1), cube_m = [], plane_g = new THREE.PlaneGeometry(2, 2, 1), plane_m = [], plane = [], textureLoader = new THREE.TextureLoader(), fsh = [fsh_px, fsh_nx, fsh_py, fsh_ny, fsh_pz, fsh_nz], promise = [];
                ...
                async function f() {
                    for (let i = 0; i < 6; i++) {
                        ...
                        if (i === 5) {
                            ...
                            cube['animate'] = (et, renderer, vrscene) => {
                                console.log(`************* cube.animate()`);
                                //vrlens animation via relative vrscene animation
                                //NOTE: by filter in narrative.addActor vrlens is not in vrscene
                                //console.log(`vrscene = ${vrscene}`);
                                //vrscene.translateZ(.002*Math.sin(.001*et));  
                                vrscene.rotateY(.001);
                                vrscene.position.z = 1.0 * Math.sin(.001 * et);
                                //vrscene.needsUpdate = true;
                            ...
                            //              cube['animate'] = (et:number, renderer:THREE.WebGLRenderer, vrscene:THREE.Scene) => {
                            //                console.log(`************* cube.animate()`);
                            //                //frame++;
                            //
                            //                //console.log(`renderer = ${renderer}`);
...
//# sourceMappingURL=mrtgts_skybox.js.map